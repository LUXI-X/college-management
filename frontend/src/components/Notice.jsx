    ...
    <div className="w-full mx-auto flex justify-center items-start flex-col my-10">
      <div className="relative flex justify-between items-center w-full">
        ...
        {(router.pathname === "/faculty" || router.pathname === "/admin") &&
          (open ? (
            <button
              className="absolute right-2 flex justify-center items-center border-2 border-cyan-500 px-3 py-2 rounded text-sky-500"
              onClick={openHandler}
            >
              <span className="mr-2">
                <BiArrowBack className="text-sky-500" />
              </span>
              Close
            </button>
          ) : (
            <button
              className="absolute right-2 flex justify-center items-center border-2 border-sky-500 px-3 py-2 rounded text-sky-500"
              onClick={openHandler}
            >
              Add Notice
              <span className="ml-2">
                <IoAddOutline className="text-sky-500 text-xl" />
              </span>
            </button>
          ))}
      </div>
          ...
          {notice &&
            notice.map((item, index) => {
              return (
                <div
                  key={item._id}
                  className="border-cyan-500 border-2 w-full rounded-md shadow-sm py-4 px-6 mb-4 relative"
                >
                  {(router.pathname === "/faculty" ||
                    router.pathname === "/admin") && (
                    <div className="absolute flex justify-center items-center right-4 bottom-3">
                      <span className="text-sm bg-cyan-500 px-4 py-1 text-white rounded-full">
                        {item.type}
                      </span>
                      <span
                        className="text-2xl group-hover:text-cyan-500 ml-2 cursor-pointer hover:text-sky-500"
                  ...
                  >
                    {item.title}
                    {item.link && (
                      <span className="text-2xl group-hover:text-cyan-500 ml-1">
                        <IoMdLink />
                      </span>
                    )}
                  </p>
      ...
      {open && (
        <form className="mt-8 w-full">
          ...
          <div className="w-[40%] mt-4">
            ...
            <select
              id="type"
              className="px-2 bg-blue-50 py-3 rounded-sm text-base w-[80%] accent-cyan-700 mt-4"
              value={data.type}
          ...
          {edit && (
            <button
              ...
              className="bg-cyan-500 text-white mt-6 px-6 rounded text-lg py-2 hover:bg-sky-400"
            >
              Update Notice
            </button>
          )}
          {!edit && (
            <button
              ...
              className="bg-cyan-500 text-white mt-6 px-6 rounded text-lg py-2 hover:bg-sky-600"
            >
              Add Notice
            </button>
          )}
